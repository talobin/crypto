package haivo.us.crypto.model.market;

import android.text.TextUtils;
import haivo.us.crypto.model.CheckerInfo;
import haivo.us.crypto.model.CurrencyPairInfo;
import haivo.us.crypto.model.Market;
import haivo.us.crypto.model.Ticker;
import java.util.List;
import org.json.JSONArray;
import org.json.JSONObject;

public class SwissCex extends Market {
    public static String API_KEY = null;
    private static final String NAME = "SWISSCEX";
    private static final String TTS_NAME = "Swiss Cex";
    private static final String URL = "http://api.swisscex.com/quote/%1$s/%2$s?apiKey=%3$s";
    private static final String URL_CURRENCY_PAIRS = "http://api.swisscex.com/quotes?apiKey=%1$s";

    static {
        API_KEY = "61u3kk4h2una357envden8cuk6";
    }

    public SwissCex() {
        super(NAME, TTS_NAME, null);
    }

    public String getUrl(int requestId, CheckerInfo checkerInfo) {
        return String.format(URL, new Object[]{checkerInfo.getCurrencyBase(), checkerInfo.getCurrencyCounter(), API_KEY});
    }

    protected void parseTickerFromJsonObject(int requestId, JSONObject jsonObject, Ticker ticker, CheckerInfo checkerInfo) throws
                                                                                                                           Exception {
        JSONObject quoteJsonObject = jsonObject.getJSONObject("quote");
        ticker.bid = quoteJsonObject.getDouble("bidPrice");
        ticker.ask = quoteJsonObject.getDouble("askPrice");
        ticker.vol = quoteJsonObject.getDouble("volume24");
        ticker.high = quoteJsonObject.getDouble("high24");
        ticker.low = quoteJsonObject.getDouble("low24");
        ticker.last = quoteJsonObject.getDouble("lastPrice");
    }

    protected String parseError(int requestId, String responseString, CheckerInfo checkerInfo) throws Exception {
        if (TextUtils.isEmpty(API_KEY)) {
            return "API_KEY is empty";
        }
        return responseString;
    }

    public String getCurrencyPairsUrl(int requestId) {
        return String.format(URL_CURRENCY_PAIRS, new Object[]{API_KEY});
    }

    protected void parseCurrencyPairsFromJsonObject(int requestId, JSONObject jsonObject, List<CurrencyPairInfo> pairs) throws
                                                                                                                        Exception {
        JSONArray pairNames = jsonObject.names();
        for (int i = 0; i < pairNames.length(); i++) {
            JSONObject marketObject = jsonObject.getJSONObject(pairNames.getString(i));
            pairs.add(new CurrencyPairInfo(marketObject.getString("from"), marketObject.getString("to"), marketObject.getString("label")));
        }
    }
}
